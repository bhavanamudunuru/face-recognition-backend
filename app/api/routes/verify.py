...
from app.utils.image_utils import download_image, check_image_quality
...
async def verify_user_face(body: VerifyFaceRequest):
    ...
    Steps: download -> quality -> detection -> liveness -> verification -> save result
    """
    scanned_path = None
    ...
    try:
        ...
        # Step 2 — Download scanned image
        scanned_path = download_image(body.image_url)

        # Step 3 — Image quality check
        quality = check_image_quality(scanned_path)
        if not quality["passed"]:
            save_scan_result(body.user_id, body.image_url, "poor_quality", message=quality["reason"])
            return FaceResult(status="poor_quality", message=quality["reason"])

        # Step 4 — Face detection
        detection = detect_face(scanned_path)
        if not detection["detected"]:
            save_scan_result(body.user_id, body.image_url, "poor_quality", message=detection["reason"])
        ...
        if not liveness["is_live"]:
            save_scan_result(body.user_id, body.image_url, "liveness_failed", message=liveness["reason"])
            return FaceResult(status="liveness_failed", message=liveness["reason"])

        # Step 6 — Download registered face and verify
        registered_path = download_image(user["face_image_url"])
        verification = verify_face(registered_path, scanned_path)

        if verification["verified"]:
            confidence = verification["confidence"]
            save_scan_result(body.user_id, body.image_url, "verified", confidence=confidence, message="Face matched successfully.")
            return FaceResult(status="verified", message="Face verified successfully.", confidence=confidence)
        else:
            save_scan_result(body.user_id, body.image_url, "not_verified", confidence=verification["confidence"], message="Face did not match registered face.")
